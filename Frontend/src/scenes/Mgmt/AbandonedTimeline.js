...
const AbandonedTimeline = () => {

  const caseID = "KVB-02";  /*To be fetched from DB*/
  const Name = "Gunjan";  /*To be fetched from DB*/
  const [selectedFile, setSelectedFile] = useState();
  const [isFilePicked, setIsFilePicked] = useState(false);


  const [step_one, setStepOne] = useState(() => {
    const storedStepOne = localStorage.getItem(`${caseID}_abandoned_step_one`);
    return storedStepOne ? JSON.parse(storedStepOne) : [
      ...
      { id: 3, text: 'File Missing Compliant', completed: false, checkboxes: [{ id: 1, checked: false }, { id: 2, checked: false }] },
      { id: 4, text: 'Final Police Report', completed: false, checkboxes: [{ id: 1, checked: false }, { id: 2, checked: false }] },
      { id: 5, text: 'Medical Report for age verification', completed: false, checkboxes: [{ id: 1, checked: false }, { id: 2, checked: false }] },
      { id: 6, text: 'Social Investigation Report', completed: false, checkboxes: [{ id: 1, checked: false }, { id: 2, checked: false }] },
    ];
  });

  const [step_two, setStepTwo] = useState(() => {
    const storedStepTwo = localStorage.getItem(`${caseID}_abandoned_step_two`);
  ...
  const [step_three, setStepThree] = useState(() => {
    const storedStepThree = localStorage.getItem(`${caseID}_abandoned_step_three`);
  ...
  const [step_four, setStepFour] = useState(() => {
    const storedStepFour = localStorage.getItem(`${caseID}_abandoned_step_four`);
  ...
  /*Depends on step*/
  useEffect(() => {
    localStorage.setItem(`${caseID}_abandoned_step_one`, JSON.stringify(step_one));
  }, [step_one]);


  useEffect(() => {
    localStorage.setItem(`${caseID}_abandoned_step_two`, JSON.stringify(step_two));
  }, [step_two]);

  useEffect(() => {
    localStorage.setItem(`${caseID}_abandoned_step_three`, JSON.stringify(step_three));
  }, [step_three]);

  useEffect(() => {
    localStorage.setItem(`${caseID}_abandoned_step_four`, JSON.stringify(step_four));
  }, [step_four]);
  ...
  return (
    <>
      ...
      <div className='all-details-timeline'>
        <div>
          <span>
            <span className='step-name'>Complete Documentation</span>  {/*Depends on step*/}
        ...
        <button className="completed-btn" onClick={changeHandler}><NavBtnLink to="/CompletedCases">Confirm as Completed</NavBtnLink></button>
      </div>
    </>
  );
};

export default AbandonedTimeline;
