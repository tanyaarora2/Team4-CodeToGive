...
const AdmittedTimeline = () => {
  ...
  const [step_one, setStepOne] = useState(() => {
    const storedStepOne = localStorage.getItem(`${caseID}_admitted_step_one`);
    return storedStepOne ? JSON.parse(storedStepOne) : [
      { id: 1, text: 'Social Investigation Report', completed: false, checkboxes: [{ id: 1, checked: false }, { id: 2, checked: false }] },
      { id: 2, text: 'Newspaper Publication', completed: false, checkboxes: [{ id: 1, checked: false }, { id: 2, checked: false }] },
      { id: 3, text: 'TV Telecasting', completed: false, checkboxes: [{ id: 1, checked: false }, { id: 2, checked: false }] },
      { id: 4, text: 'File Report with Police Station to Trace Guardians ', completed: false, checkboxes: [{ id: 1, checked: false }, { id: 2, checked: false }] },
      { id: 5, text: 'Final Police Report', completed: false, checkboxes: [{ id: 1, checked: false }, { id: 2, checked: false }] },
  ...
  const [step_two, setStepTwo] = useState(() => {
    const storedStepTwo = localStorage.getItem(`${caseID}_admitted_step_two`);
  ...
  const [step_three, setStepThree] = useState(() => {
    const storedStepThree = localStorage.getItem(`${caseID}_step_admitted_three`);
  ...
  const [step_four, setStepFour] = useState(() => {
    const storedStepFour = localStorage.getItem(`${caseID}_admitted_step_four`);
  ...
  useEffect(() => {
    ...
    const storedStepThree = localStorage.getItem(`${caseID}_admitted_step_three`);
    if (storedStepThree) {
  ...
  /*Depends on step*/
  useEffect(() => {
    localStorage.setItem(`${caseID}_admitted_step_one`, JSON.stringify(step_one));
  }, [step_one]);


  useEffect(() => {
    localStorage.setItem(`${caseID}_admitted_step_two`, JSON.stringify(step_two));
  }, [step_two]);

  useEffect(() => {
    localStorage.setItem(`${caseID}_admitted_step_three`, JSON.stringify(step_three));
  }, [step_three]);

  useEffect(() => {
    localStorage.setItem(`${caseID}_admitted_step_four`, JSON.stringify(step_four));
  }, [step_four]);
...
export default AdmittedTimeline;
