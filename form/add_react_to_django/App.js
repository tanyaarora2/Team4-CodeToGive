...
const ManagementTabsPage = () => {
  ...
  // const ActionNeededDetails = (item, {navigation}) => {
  //   //console.log(`Button clicked for row ${item.Case_Number}`);
  //   return (
  ...
  const renderTableRowsActionNeeded = () => {
    const navigateToActionNeeded = (caseNumber) => {
      navigate(`./ActionNeeded.js?id=${caseNumber}`);
    };
          ...
          <tbody>
            {data.map((item, index) => (
              <tr key={item.Case_Number}>
                ...
                {item.Action === Boolean(true) && (
                  <td>
                    <button className='details-btn' onClick={() => navigateToActionNeeded(item.Case_Number)}>
                      Details
                    </button>
                  </td>
                )}
